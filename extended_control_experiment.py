...
def run_control_seed(model_id: str, model_type: str, seed: int) -> Dict[str, object]:
...
def aggregate_seeds(results: List[Dict]) -> Dict[str, object]:
    before_pcts = [r["before"]["laplace_pct"] for r in results]
    after_pcts = [r["after"]["laplace_pct"] for r in results]
    
    return {
        "before_mean": float(np.mean(before_pcts)),
        "before_std": float(np.std(before_pcts)),
        "after_mean": float(np.mean(after_pcts)),
        "after_std": float(np.std(after_pcts)),
        "change_mean": float(np.mean([a - b for a, b in zip(after_pcts, before_pcts)])),
...
def main() -> None:
    ...
    print("=" * 96)
    print("EXTENDED RANDOMIZED-LABEL CONTROL EXPERIMENT")
    print("=" * 96)
    ...
    for model_id, model_type in CONTROL_MODELS:
        try:
            ...
            for seed in range(NUM_SEEDS):
                result = run_control_seed(model_id, model_type, seed)
                seed_results.append(result)
                print(f"  seed {seed}: {result['before']['laplace_pct']:.1f}% → {result['after']['laplace_pct']:.1f}%")
            ...
            print(f"  average: {aggregated['before_mean']:.1f}% (±{aggregated['before_std']:.1f}) → {aggregated['after_mean']:.1f}% (±{aggregated['after_std']:.1f})")
            print(f"  mean change: {aggregated['change_mean']:.2f}%\n")
        except Exception as exc:
            print(f"  failed: {exc}\n")

    out_path = os.path.join(os.getcwd(), "extended_control_results.json")
    with open(out_path, "w", encoding="utf-8") as handle:
        json.dump({"results": all_results}, handle, indent=2)

    print("=" * 96)
    print(f"Saved extended control results to {out_path}")
    print("\nSummary:")
    for item in all_results:
        agg = item["aggregated"]
        print(f"  {item['model_id']}: {agg['before_mean']:.1f}% → {agg['after_mean']:.1f}% (Δ {agg['change_mean']:.2f}%)")
